...
input_x = np.array([[1,0],[1,1],[0,0]])

## OUTPUT
actual_output=np.array([ [1],[0],[0] ])
...
bias_0=np.array(np.random.randn(3,1))
bias_1=np.array(np.random.randn(3,1))
...
for i in range(1):
    ...
    l1 = sigmoid(np.dot(l0, weights_0)+bias_0)
    
    #LAYER 2 IS l1 * WEIGHTS_1 and ITS SIGMOID
    l2 = sigmoid(np.dot(l1, weights_1)+bias_1)
...
print("Predicting reponse for 0 1")
